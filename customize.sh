...
#
INI_1_WMSHOST='wms://infn-wms-01.ct.pi2s2.it:7443/glite_wms_wmproxy_server'

#init_pxServerHost
INI_1_PXHOST='myproxy.ct.infn.it'
...
#nameInfrastructure
INI_2_NAMEINFRA='COMETA Consortium infrastructure'

#acronymInfrastructure
INI_2_ACRONYM='COMETA'
...
#init_bdiiHost
# specify the information system BDII in the form ldap://<info_provider_hostname>:<info_provide_port>
INI_2_BDIIHOST='ldap://infn-bdii-01.ct.pi2s2.it:2170'
...
#init_pxServerHost
INI_2_PXHOST='myproxy.ct.infn.it'
...
#init_pxRobotVO
INI_2_ROBOTVO=cometa

#init_pxRobotRole
INI_2_ROBOROLE=cometa
...
#init_bdiiHost
# specify the information system BDII in the form ldap://<info_provider_hostname>:<info_provide_port>
INI_3_BDIIHOST='ldap://bdii.eela.ufrj.br:2170'

#init_wmsHost
# The wms host can be obtained from an UI with the gLite command line
# lcg-infosites --vo eumed wms
#
INI_3_WMSHOST='wms://wms.eela.ufrj.br:7443/glite_wms_wmproxy_server'

#init_pxServerHost
INI_3_PXHOST='myproxy.ct.infn.it'
...
#
cat docroot/WEB-INF/src/it/infn/ct/mi_hostname_portlet.java | sed s/"mi_hostname_portlet"/"${CLASS_NAME}"/g | sed s/"@author <a href=\"mailto:riccardo.bruno@ct.infn.it\">riccardo bruno<\/a>(COMETA)"/"@author <a href=\"mailto:${AUTH_EMAIL}\">${AUTH_NAME}<\/a>(${AUTH_INSTITUTE})"/ | sed s/"hostname-Output.txt"/"${APP_NAME}-Output.txt"/ | sed s/"hostname-Error.txt"/"${APP_NAME}-Output.txt"/ | sed s/"hostname-Files.tar.gz"/"${APP_NAME}-Files.tar.gz"/ > docroot/WEB-INF/src/$(echo $BASE_CLASS | sed s/'\.'/'\/'/g)/${CLASS_NAME}.java
